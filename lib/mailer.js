...
const mailerConfig = require('../config/mailer').config() // TODO: コンフィグの位置は固定したくない
  ...
  const send = async({key, ...params}) => {
    ...
    if(key) {
      ...
      if(params.text === undefined) {
        params.text = await render(path.join(params.rootDir, `${key}.text.ejs`))
      }
      if(params.html === undefined) {
        params.html = await render(path.join(params.rootDir, `${key}.html.ejs`))
      }
    }
